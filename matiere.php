<?php
    ...
    $requeteVerifMatiere = $bdd->prepare("SELECT ecole, promo, nomMatiere FROM Matiere WHERE idMatiere = :idMatiere");
    $requeteVerifMatiere->bindParam(':idMatiere', $idMatiere);
    $requeteVerifMatiere->execute();

    $verifMatiere = $requeteVerifMatiere->fetch(PDO::FETCH_ASSOC);

    if($verifMatiere['ecole'] != $_SESSION['ecole'] || $verifMatiere['promo'] != $_SESSION['promo']){
